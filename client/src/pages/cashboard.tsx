      ...
      <main className="flex-1 flex flex-col min-h-0 main-with-bottom-nav">
        ...
        <div className="flex-1 flex flex-col min-h-0 overflow-y-auto">
          ...
          <section className="hidden sm:block mb-6 flex-shrink-0">
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-4">State of Cash</p>
            <div className="grid grid-cols-4 gap-8">
              {/* Total Outstanding */}
              <div data-testid="card-total-outstanding">
                <p className="text-[12px] text-muted-foreground mb-1">Total Outstanding</p>
                {metricsLoading ? (
                  <div className="h-6 w-24 bg-muted animate-pulse rounded"></div>
                ) : (
                  <p className="text-[20px] font-semibold text-foreground tabular-nums">
                    {formatCurrency(totalOutstanding)} <span className="text-[12px] font-normal text-muted-foreground">({totalInvoiceCount})</span>
                  </p>
                )}
              </div>
              ...
              <div data-testid="card-overdue">
                <p className="text-[12px] text-muted-foreground mb-1">Overdue</p>
                {metricsLoading ? (
                  <div className="h-6 w-24 bg-muted animate-pulse rounded"></div>
                ) : (
                  <p className="text-[20px] font-semibold text-foreground tabular-nums">
                    {formatCurrency(overdueAmount)} <span className="text-[12px] font-normal text-muted-foreground">({overdueCount})</span>
                  </p>
                )}
              </div>
              ...
              <div data-testid="card-collected-month">
                <p className="text-[12px] text-muted-foreground mb-1">Collected This Month</p>
                {metricsLoading ? (
                  <div className="h-6 w-24 bg-muted animate-pulse rounded"></div>
                ) : (
              ...
              <div data-testid="card-collected-week">
                <p className="text-[12px] text-muted-foreground mb-1">Collected This Week</p>
                {metricsLoading ? (
                  <div className="h-6 w-24 bg-muted animate-pulse rounded"></div>
                ) : (
                  <p className="text-[20px] font-semibold text-emerald-600 tabular-nums">
          ...
          <section className="sm:hidden mb-6 flex-shrink-0">
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-3">State of Cash</p>
            <div className="grid grid-cols-2 gap-4">
              <div data-testid="card-total-outstanding">
                <p className="text-[11px] text-muted-foreground">Outstanding</p>
                <p className="text-[16px] font-semibold text-foreground tabular-nums">{formatCurrency(totalOutstanding)}</p>
              </div>
              <div data-testid="card-overdue">
                <p className="text-[11px] text-muted-foreground">Overdue</p>
                <p className="text-[16px] font-semibold text-foreground tabular-nums">{formatCurrency(overdueAmount)}</p>
              </div>
              <div data-testid="card-collected-month">
                <p className="text-[11px] text-muted-foreground">Collected (Month)</p>
                <p className="text-[16px] font-semibold text-emerald-600 tabular-nums">{formatCurrency(metrics?.collectedThisMonth || 0)}</p>
              </div>
              <div data-testid="card-collected-week">
                <p className="text-[11px] text-muted-foreground">Collected (Week)</p>
          ...
          {/* Hairline divider */}
          <div className="border-t border-border/50/80 mb-6 flex-shrink-0" />
          ...
          <section className="flex-1 flex flex-col min-h-0 mb-6" data-testid="card-cashflow-chart">
            <div className="flex items-center justify-between mb-3 flex-shrink-0">
              <h3 className="text-[14px] font-medium text-foreground">{getForecastTitle()}</h3>
                  ...
                  <ComposedChart data={formatChartData()}>
                    ...
                    <RechartsTooltip
                      content={({ active, payload, label }) => {
                        ...
                        if (!data) return null;
                        return (
                          <div className="bg-white border border-border rounded-md p-3 shadow-sm text-[12px]">
                            ...
                            <div className="space-y-1">
                              ...
                              <div className="flex justify-between gap-4">
                                ...
                                <span className="font-medium tabular-nums text-foreground">{formatCurrency(data.confidenceWeightedAmount)}</span>
                              </div>
                              <div className="border-t border-border/50 my-2 pt-2">
                                <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1">By Confidence</p>
                                <div className="flex justify-between gap-4">
          ...
          <section className="flex-shrink-0 pb-4">
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-4">Behaviour Performance</p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 sm:gap-8">
              {/* Avg Days Late */}
              <div data-testid="card-perf-avg-days-late">
                <p className="text-[12px] text-muted-foreground mb-1">Avg Days Late</p>
                {metricsLoading ? (
                  <div className="h-6 w-16 bg-muted animate-pulse rounded"></div>
                ) : (
                  <p className="text-[20px] font-semibold text-foreground tabular-nums">
                    {Math.abs(avgDaysOverdue).toFixed(0)} <span className="text-[12px] font-normal text-muted-foreground">days</span>
                  </p>
                )}
              </div>
              ...
              <div data-testid="card-perf-avg-days-pay">
                <p className="text-[12px] text-muted-foreground mb-1">Avg Days to Pay</p>
                {metricsLoading ? (
                  <div className="h-6 w-16 bg-muted animate-pulse rounded"></div>
                ) : (
                  <p className="text-[20px] font-semibold text-foreground tabular-nums">
                    45 <span className="text-[12px] font-normal text-muted-foreground">days</span>
                  </p>
                )}
              </div>
              ...
              <div data-testid="card-perf-ontime-rate">
                <p className="text-[12px] text-muted-foreground mb-1">On-time Rate</p>
                {metricsLoading ? (
                  <div className="h-6 w-16 bg-muted animate-pulse rounded"></div>
                ) : (
                  <p className="text-[20px] font-semibold text-emerald-600 tabular-nums">
              ...
              <div data-testid="card-perf-promises-kept">
                <p className="text-[12px] text-muted-foreground mb-1">Promises Kept</p>
