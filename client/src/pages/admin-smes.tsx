      ...
      <div className="sticky top-0 z-40 bg-background border-b border-border">
        <div className="px-6 lg:px-8 py-5 flex items-center justify-between">
          <div>
            ...
            <p className="text-[13px] text-muted-foreground/60 mt-0.5">All tenant businesses on the platform</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 lg:p-8">
        {isLoading ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="text-[13px] text-muted-foreground/60">Clients will appear here when they connect</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left text-[11px] font-medium text-muted-foreground/60 uppercase tracking-wider py-3 pr-4">Client</th>
                  <th className="text-left text-[11px] font-medium text-muted-foreground/60 uppercase tracking-wider py-3 px-4">Xero</th>
                  <th className="text-left text-[11px] font-medium text-muted-foreground/60 uppercase tracking-wider py-3 px-4">Last sync</th>
                  <th className="text-left text-[11px] font-medium text-muted-foreground/60 uppercase tracking-wider py-3 px-4">Comms</th>
                  <th className="text-center text-[11px] font-medium text-muted-foreground/60 uppercase tracking-wider py-3 px-4">Actions</th>
                  <th className="text-left text-[11px] font-medium text-muted-foreground/60 uppercase tracking-wider py-3 px-4">Created</th>
                  <th className="text-center text-[11px] font-medium text-muted-foreground/60 uppercase tracking-wider py-3 pl-4 w-10"></th>
                </tr>
              </thead>
              <tbody>
                {clients.map((client) => (
                  <tr
                    key={client.id}
                    className="border-b border-slate-50 hover:bg-muted/50 transition-colors"
                  >
                    <td className="py-3 pr-4">
                      ...
                      {client.xeroOrganisationName && client.xeroOrganisationName !== client.name && (
                        <span className="text-[12px] text-muted-foreground/60 ml-2">({client.xeroOrganisationName})</span>
                      )}
                    </td>
                    <td className="py-3 px-4">{getXeroStatus(client)}</td>
                    <td className="py-3 px-4 text-[13px] text-muted-foreground/60">{formatDateTime(client.xeroLastSyncAt)}</td>
                    ...
                    <td className="py-3 px-4 text-[13px] text-muted-foreground/60">{formatDate(client.createdAt)}</td>
