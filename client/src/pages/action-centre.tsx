                ...
                <>
                  ...
                  <div className="lg:hidden flex-1 overflow-y-auto">
                    <div className="space-y-4 p-4">
                      {queueData.map((action: QueueDisplayItem, index: number) => {
                        const isSelected = selectedItems.has(action.id);
                        const overdueDays = isInvoiceItem(action) 
                          ? Math.max(0, Math.floor((new Date().getTime() - new Date(action.dueDate).getTime()) / (1000 * 60 * 60 * 24)))
                          : 0;
                        
                        return (
                          <div 
                            key={action.id}
                            className={`bg-white/80 backdrop-blur-sm border border-white/50 rounded-lg shadow-lg p-4 transition-all duration-300 ${
                              isSelected
                                ? 'ring-2 ring-[#17B6C3] bg-[#17B6C3]/10'
                          ...
                          >
                            ...
                            <div className="flex items-start justify-between mb-3">
                              <div className="flex items-center space-x-3">
                                <Checkbox
                                  ...
                                  onCheckedChange={() => handleSelectItem(action.id, index)}
                                  className="mt-1"
                                  data-testid={`checkbox-mobile-${action.id}`}
                                />
                                <div>
                                  ...
                                  <p className="text-sm text-slate-600">
                                    {isInvoiceItem(action) ? `Invoice #${action.invoiceNumber || 'N/A'}` : 'Action Item'}
                                  </p>
                                </div>
                              </div>
                              ...
                              {/* Priority Badge */}
                              <div className="flex items-center space-x-2">
                                {isInvoiceItem(action) && (
                                  <Badge 
                                    variant={overdueDays > 30 ? "destructive" : overdueDays > 0 ? "secondary" : "default"}
                                    className="text-xs"
                                  >
                                    {overdueDays > 0 ? `${overdueDays}d overdue` : 'Due'}
                                  </Badge>
                                )}
                              </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-3 mb-4 text-sm">
                              {isInvoiceItem(action) && (
                                <>
                                  <div>
                                    ...
                                    <p className="font-semibold text-lg text-[#17B6C3]">
                                      {(() => {
                                        try {
                                          let amount = 0;
                                          if ('totalAmount' in action && action.totalAmount) {
                              ...
                              <div className="col-span-2">
                                ...
                                <p className="font-medium">{action.type || 'Payment Reminder'}</p>
                              </div>
                            </div>

                            {/* Mobile Action Buttons */}
                            <div className="grid grid-cols-2 gap-3">
                              {/* Payment Promise - Primary Action */}
                              <Button
                                variant="default"
                                className="bg-[#17B6C3] hover:bg-[#1396A1] text-white h-12 font-semibold"
                                onClick={(e) => {
                                  e.stopPropagation();
                                  toast({
                                    title: "Payment Promise",
                                ...
                                data-testid={`button-mobile-promise-${action.id}`}
                              >
                                <CheckCircle className="h-5 w-5 mr-2" />
                                Promise
                              </Button>

                              {/* AI Call - Secondary Action */}
                              <Button
                                variant="outline"
                                className="border-[#17B6C3]/20 text-[#17B6C3] hover:bg-[#17B6C3]/5 h-12"
                                onClick={async (e) => {
                                  e.stopPropagation();
                                  const contactId = action.contactId;
                                  if (contactId) {
                                    try {
                                      ...
                                      if (response.ok) {
                                        ...
                                        toast({
                                          title: "Contact Not Found",
                                          description: "The contact for this action no longer exists.",
                                          variant: "destructive",
                                        });
                                      }
                                    } catch (error) {
                                ...
                                data-testid={`button-mobile-call-${action.id}`}
                              >
                              ...
                              <Button
                                variant="outline"
                                className="border-gray-200 text-slate-700 hover:bg-slate-50 h-12"
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setCommunicationDialog({
                                    isOpen: true,
                                ...
                                data-testid={`button-mobile-email-${action.id}`}
                              >
                              ...
                              <Button
                                ...
                                data-testid={`button-mobile-sms-${action.id}`}
                              >
                            ...
                            <div className="mt-3 pt-3 border-t border-gray-100">
                              <div className="flex justify-between items-center">
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  className="text-[#17B6C3] hover:bg-[#17B6C3]/5"
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    setPaymentPlanAction(action);
                                    setShowPaymentPlanDialog(true);
                                  }}
                                  data-testid={`button-mobile-plan-${action.id}`}
                                >
                                  <CalendarIcon className="h-4 w-4 mr-2" />
                                  Payment Plan
                                </Button>
                                ...
                                <Button
                                  ...
                                  data-testid={`button-mobile-details-${action.id}`}
                                >
