...
export default function ActionCentre() {
  ...
  // Fetch completed actions for AI Activity Feed
  ...
  const { data: completedActions = [] } = useQuery<CompletedAction[]>({
    queryKey: ['/api/actions?status=completed'],
    enabled: activeTab === 'plan',
  });
              ...
              {!isLoadingPlan && dailyPlan && dailyPlan.actions.length > 0 && (
                <>
                  ...
                  {completedActions.filter(a => {
                    ...
                    <div className="bg-teal-50/80 backdrop-blur-sm border border-teal-200 rounded-xl p-5 shadow-lg">
                      <div className="flex items-center gap-3 mb-4">
                        <div className="p-2 bg-teal-100 rounded-lg">
                          <CheckCircle2 className="h-5 w-5 text-teal-600" />
                        </div>
                        <div>
                          <h3 className="text-lg font-semibold text-slate-900">AI Completed Today</h3>
                          <p className="text-sm text-slate-600">Actions executed autonomously by the system</p>
                        </div>
                      </div>
                      <div className="divide-y divide-teal-100 max-h-48 overflow-y-auto">
                        {completedActions.filter(a => {
                          ...
                          const IconComponent = item.type === 'email' ? Mail : item.type === 'voice' ? Phone : MessageSquare;
                          return (
                            <div 
                              key={item.id} 
                              className="flex items-center gap-4 py-2 px-2 -mx-2"
                              data-testid={`completed-row-${idx}`}
                            >
                              <div className="p-1.5 rounded-lg shrink-0 bg-teal-100">
                                <IconComponent className="h-3.5 w-3.5 text-teal-600" />
                              </div>
                              <div className="flex-1 min-w-0">
                                <span className="text-sm text-slate-700 truncate">{item.companyName || item.contactName || 'Customer'}</span>
                              </div>
                              <span className="text-xs text-slate-500">{getSmartTimestamp(item.completedAt)}</span>
                              {item.invoiceAmount && (
                                <span className="text-sm font-medium text-slate-700 tabular-nums">{formatCurrency(parseFloat(item.invoiceAmount))}</span>
                              )}
                            </div>
                          );
                        })}
                  ...
                  {/* Exceptions Section - Items needing review */}
                  {dailyPlan.actions.filter(a => a.status === 'exception').length > 0 && (
                    <div className="bg-amber-50/80 backdrop-blur-sm border border-amber-200 rounded-xl p-5 shadow-lg">
                      <div className="flex items-center gap-3 mb-4">
                        <div className="p-2 bg-amber-100 rounded-lg">
                          <AlertTriangle className="h-5 w-5 text-amber-600" />
                        </div>
                        <div>
                          <h3 className="text-lg font-semibold text-slate-900">Needs Your Review</h3>
                          <p className="text-sm text-slate-600">These items require human attention before AI can proceed</p>
                        </div>
                      </div>
                      <div className="divide-y divide-amber-100">
                        {dailyPlan.actions.filter(a => a.status === 'exception').map((item, idx) => {
                          const IconComponent = item.actionType === 'email' ? Mail : item.actionType === 'voice' ? Phone : MessageSquare;
                          return (
                            <div 
                              key={item.id} 
                              className="flex items-center gap-4 py-3 hover:bg-amber-100/50 px-2 -mx-2 rounded transition-colors cursor-pointer"
                              onClick={() => {
                                setSelectedPlanAction(item);
                                setIsPreviewDrawerOpen(true);
                              }}
                              data-testid={`exception-row-${idx}`}
                            >
                              <div className="p-2 rounded-lg shrink-0 bg-amber-100">
                              ...
                              <Badge className="bg-amber-100 text-amber-700 border-amber-200 text-xs">
                                {item.exceptionReason || 'Review Required'}
                              </Badge>
                              <span className="text-sm font-semibold text-slate-900 tabular-nums shrink-0">{formatCurrency(parseFloat(item.amount))}</span>
                              <Button
                                size="sm"
                                className="h-8 bg-[#17B6C3] hover:bg-[#1396A1] text-white text-xs"
                                onClick={(e) => {
                                  e.stopPropagation();
                                  approveActionMutation.mutate(item.id);
                                }}
                                disabled={approveActionMutation.isPending}
                                data-testid={`button-resolve-exception-${idx}`}
                              >
                                Approve
                              </Button>
                            </div>
                  ...
                  {/* Approved/Scheduled Section - AI will handle these */}
                  {dailyPlan.actions.filter(a => a.status === 'scheduled').length > 0 && (
                    <div className="bg-green-50/80 backdrop-blur-sm border border-green-200 rounded-xl p-5 shadow-lg">
                      <div className="flex items-center gap-3 mb-4">
                        <div className="p-2 bg-green-100 rounded-lg">
                          <CheckCircle2 className="h-5 w-5 text-green-600" />
                        </div>
                        <div>
                          <h3 className="text-lg font-semibold text-slate-900">AI Will Execute</h3>
                          <p className="text-sm text-slate-600">These actions are approved and will run at {dailyPlan.tenantPolicies?.executionTime || '09:00'}</p>
                        </div>
                      </div>
                      <div className="divide-y divide-green-100">
                        {dailyPlan.actions.filter(a => a.status === 'scheduled').map((item, idx) => {
                          const IconComponent = item.actionType === 'email' ? Mail : item.actionType === 'voice' ? Phone : MessageSquare;
                          ...
                          return (
                            ...
                            >
                              ...
                              <span className="text-xs text-slate-500">{item.daysOverdue}d overdue</span>
                              <span className="text-sm font-medium text-slate-700 tabular-nums">{formatCurrency(parseFloat(item.amount))}</span>
                            </div>
                          );
                        })}
                      </div>
                  ...
                  {/* Pending Approval Section - Awaiting user decision */}
                  {dailyPlan.actions.filter(a => a.status === 'pending_approval').length > 0 && (
                    <div className="bg-white/80 backdrop-blur-sm border border-white/50 rounded-xl p-5 shadow-lg">
