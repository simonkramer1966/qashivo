...
-   **Automated Report Scheduling**: Users can schedule automated report generation (Aged Debtors, Cash Flow Forecast, Collection Performance, DSO Summary) with daily/weekly/monthly frequency. Reports are generated as HTML emails and delivered via SendGrid to configured recipients. Managed in Settings > Reports tab. Key files: `server/services/reportGenerator.ts`, `server/services/reportScheduler.ts`, `client/src/components/settings/ReportSchedulesTab.tsx`. RBAC: `admin:settings` permission required.
...
-   **6-Step Onboarding Wizard**: Skippable multi-step onboarding: (1) Company Details (required), (2) Connect Xero (skippable), (3) Connect Email (skippable), (4) Connect Bank (skippable MVP), (5) Debtor Scoring (non-blocking background), (6) Contact Data Analysis. Step 5 runs as a background job after Xero sync. Onboarding completes when step 1 is COMPLETED and all others are COMPLETED, SKIPPED, or RUNNING. Key files: `client/src/components/OnboardingWizard.tsx`, `client/src/components/onboarding/Step*.tsx`, `server/services/onboardingService.ts`. Settings page includes "Finish Onboarding" banner + restart/re-run controls.
-   **Debtor Scoring & Strategy Assignment**: Deterministic weighted scoring (0-100) computed from invoice payment history. Metrics: onTimeRate, avgDaysLate, p90DaysLate, late30PlusRate, volatility, paymentRecency. Bands: EXCELLENT (>=85), GOOD (70-84), OK (50-69), RISKY (<50), UNKNOWN (<3 invoices). Each band maps to a strategy template (GENTLE/STANDARD/STANDARD+/FIRM). Runs as background job via `analysis_jobs` table, triggered after Xero sync or manually from Settings. Results stored in `debtor_profiles` table with score factors JSON for explainability. Key files: `server/services/scoring/debtorScoring.ts`, `server/jobs/debtorScoringJob.ts`.
