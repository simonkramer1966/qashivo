...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/test/sendgrid', isAuthenticated, async (req, res) => {
    try {
      const { sendEmail, DEFAULT_FROM, DEFAULT_FROM_EMAIL } = await import("./services/sendgrid");
      
      console.log(`🧪 Testing SendGrid configuration with sender: ${DEFAULT_FROM}`);
      
      const user = req.user as any;
      const testRecipient = user.email || DEFAULT_FROM_EMAIL; // Fallback to sender email if user email not available
        ...
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <div style="background: #f8f9fa; padding: 20px; border-left: 4px solid #17B6C3; margin: 20px 0;">
              ...
              <strong>From Address:</strong> ${DEFAULT_FROM_EMAIL}<br>
              <strong>From Name:</strong> ${DEFAULT_FROM.split('<')[0].trim()}<br>
              <strong>Test Recipient:</strong> ${testRecipient}
            </div>
            <p><em>Generated by Nexus AR Collections System</em></p>
          </div>
        `
      });
  ...
  app.post("/api/invoices/:invoiceId/send-email", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const emailSent = await sendEmail({
        ...
        from: (defaultSender.email || defaultSender.name || DEFAULT_FROM_EMAIL) as string,
