...
export type OutcomeType = 
  ...
  | 'vulnerability'          // Vulnerability indicator detected
  | 'financial_hardship'     // Financial hardship mentioned
...
class CommunicationOutcomeProcessor {
    ...
    try {
      ...
      switch (outcome.outcomeType) {
        ...
        case 'financial_hardship':
          await this.handleVulnerability(outcome, result);
          break;
  ...
  /**
   * Handle vulnerability/financial hardship
   */
  private async handleVulnerability(outcome: OutcomeData, result: ProcessingResult): Promise<void> {
    const contact = await storage.getContact(outcome.contactId, outcome.tenantId);
    if (!contact) return;

    const actionSource = outcome.idempotencyKey 
      ? `outcome_processor_${outcome.idempotencyKey}` 
      : `outcome_processor_vuln_${outcome.contactId}`;
    ...
    const action = await this.createActionIfNotExists(actionSource, {
      ...
      subject: `🔴 URGENT: Vulnerability Indicator - ${contact.companyName || contact.name}`,
      content: `Vulnerability or financial hardship indicator detected during ${outcome.channel} communication. All automated contact PAUSED. Human review required before any further action. Notes: ${outcome.notes || 'None'}`,
      metadata: {
    ...
    if (action) {
      result.actionsCreated.push(action.id);
      result.stateUpdates.push('Automation paused - vulnerability flag');
    }
  }
