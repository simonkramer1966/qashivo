...
- 2026-04-09: URGENT FIX — Xero token refresh race between `SyncOrchestrator` and `xeroHealthCheckService`. Scheduled sync at ~12:14 failed with 403 after a 09:41 success. Root cause: **TWO uncoordinated refresh mutex Maps** — `refreshLocks` in `server/sync/adapters/XeroAdapter.ts` and `refreshInProgress` in `server/services/xero.ts`. They did not block each other. Compounding this, `xeroHealthCheckService.checkTenantConnection` **unconditionally** called `xeroService.refreshAccessToken()` every 20 minutes (cron `'*/20 * * * *'` plus a startup kick at `xeroHealthCheck.ts:13`) regardless of actual token expiry — rotating the refresh token just to verify the connection works. If a sync ran concurrently (boot kick, scheduled slot, or long pagination), both code paths could POST to Xero's identity endpoint with the same `RT_A`. Xero rotates to `RT_B` for the winner and immediately invalidates `RT_A`; the loser's request fails with `invalid_grant`, `markConnectionExpired()` fires, and the next sync sees a bad connection. The most likely 12:14 trigger was the 12:20 health-check cron overlapping a long 12:14 sync — or the boot race after today's deploy (startup health check + 10s boot-kick sync running concurrently). Fix in four parts: **(1)** New module `server/services/xeroTokenLock.ts` exposing `withXeroRefreshLock(tenantId, fn)` — a process-wide per-tenant serialization primitive backed by a single `Map<string, Promise<unknown>>`. Waiters block on the in-flight promise without adopting its return value (the two callers return different types, so each re-reads from the DB post-wait). **(2)** `XeroAdapter.refreshToken()` now wraps `doRefreshToken` in `withXeroRefreshLock`. After acquiring the lock it re-reads the tenant row — if another caller already refreshed while we were waiting, we return those fresh tokens instead of sending a second (now-doomed) refresh. Deleted the old `refreshLocks` module-level Map. **(3)** `xeroService.refreshAccessToken()` wraps `doRefreshAccessToken` in the same shared lock, similarly re-reads the tenant row post-wait, and crucially reloads `refreshToken` from the DB (the one passed as an argument may be stale if we blocked for a while). Deleted the old private `refreshInProgress` Map field. **(4)** `xeroHealthCheckService.checkTenantConnection` no longer rotates tokens unconditionally. It checks `xeroExpiresAt` with a 2-minute buffer; if the access token is still valid, it verifies API access with a lightweight `/Invoices?page=1&pageSize=1` ping using the existing token — a successful ping proves the connection works without touching the refresh token. Only when the token is actually expired does it call `refreshAccessToken` (now going through the shared lock). This eliminates ~95% of the rotation pressure on the refresh token and removes the 20-minute cadence of race windows. Both `XeroAdapter.doRefreshToken` and `xeroService.doRefreshAccessToken` already persisted rotation correctly, so no change there. Files: `server/services/xeroTokenLock.ts` (new), `server/sync/adapters/XeroAdapter.ts`, `server/services/xero.ts`, `server/services/xeroHealthCheck.ts`. Zero new type errors in touched files.

- 2026-04-09: Fixed daily sync schedule + sidebar manual-sync button. Replaced the 4-hour `setInterval` in `SyncOrchestrator.start()` with a 60-second tick that checks `crossedScheduledSlot(lastCheck, now, slots, tz)` for each connected tenant. Tenants get a new `syncScheduleTimes text[]` column on `tenants` (default `['07:00','13:00']`) interpreted in the tenant's `executionTimezone`. Pure helpers `crossedScheduledSlot`, `nextOccurrenceOfSlot`, `mostRecentSlotAtOrBefore`, `localToUtc` use `Intl.DateTimeFormat` with iterative refinement to survive DST — spring-forward gap times are skipped. New `SyncOrchestrator.getNextScheduledSyncAt(slots, tz)` powers the "Next sync HH:MM" hint. `/api/sync/current` extended with `syncScheduleTimes`, `executionTimezone`, `nextScheduledSyncAt`. New `GET/PUT /api/xero/sync/schedule` (PUT gated `withMinimumRole('manager')`, validates 1–12 entries against `/^([01]\d|2[0-3]):[0-5]\d$/`, dedupes + sorts). `POST /api/xero/sync` now gated `withMinimumRole('manager')` so non-managers can't trigger manual syncs. New shared hook `client/src/hooks/useManualSync.ts` consolidates the duplicate retryMutation from `SidebarSyncIndicator` and `SyncStatusBanner`, sniffs the "409:" message prefix from `apiRequest` for "Sync already in progress" toast. `SidebarSyncIndicator` expanded state now shows two lines (Last sync … / Next sync HH:MM) plus a role-gated manual-sync icon button that triggers the sync without navigating. Collapsed-mode tooltip shows next sync on a second line; for managers the dot tap starts a sync, for non-managers it still navigates to integrations. Settings → Integrations Xero card gains a "Sync schedule" row with comma-separated inline editor (manager+ only), reads current schedule from `/api/sync/current` (no extra round-trip), invalidates the query on save so the sidebar hint updates within a second. The legacy `xeroSyncInterval` column and `/api/xero/sync/settings` GET/PUT are left in place with a `TODO: remove` comment — nothing on the client reads them after this PR. Baseline type errors: 513 → 513. Files: `shared/schema.ts`, `server/sync/SyncOrchestrator.ts`, `server/routes/syncRoutes.ts`, `server/routes/integrationRoutes.ts`, `client/src/hooks/useSyncStatus.tsx`, `client/src/hooks/useManualSync.ts` (new), `client/src/components/sync/SidebarSyncIndicator.tsx`, `client/src/components/sync/SyncStatusBanner.tsx`, `client/src/pages/settings/integrations.tsx`. Schema change: `npm run db:push` required.

- 2026-04-07: Consolidated all debtor-facing email generation onto `debtorEmailService.generateDebtorEmail()` (Phases 1–5). Every outbound email to a debtor — pre-generated chase, executor fallback, inbound reply, active conversation reply, ambiguity clarification — now flows through one function where persona, chase context, conversation brief, conditional HTML invoice table, and the strict Name/Title/Company signature are enforced. Phase 4 rewired `emailClarificationService.sendClarificationEmail()` off a static "Accounts Team"-signed template and onto `generateDebtorEmail({ emailType: 'clarification', invoiceIds: [] })`, loading the debtor's original inbound message body from `inboundMessages` and mapping `ambiguityType` → `'date' | 'amount' | 'both'`. `buildEmailTypeAddendum()` in `debtorEmailService.ts` gained a `clarificationContext` branch: <100 words, no invoice table, persona sign-off, reference the debtor's own words. Phase 5 cleaned up dead code: deleted `analyzePaymentPatterns` and `detectSmsIntent` from `openai.ts` (both orphaned), deleted `buildClarificationEmail` + `generateDefaultQuestions` dead private methods from `emailClarificationService.ts`, and removed dead static `from "../services/openai"` imports from 7 route files (onboarding, collections, dashboard, contact, settings, integration, invoice) — each file imported `generateCollectionSuggestions, generateEmailDraft, generateAiCfoResponse` but never called any of them (historical copy-paste). `openai.ts` kept for the 4 still-live paths (`generateAiCfoResponse` via miscRoutes, `generateCollectionSuggestions` / `generateEmailDraft` via root routes.ts, `generateCollectionSms` via contactRoutes, plus `generateCollectionEmail` used by the manual draft endpoint `/api/contacts/:id/generate-email` via dynamic import). `aiMessageGenerator.ts` kept for SMS and voice (still called by actionExecutor and messagePreGenerator); its email path is dead but deletion deferred to a future cleanup. Zero new type errors (baseline 224 → 224). Files: `server/services/openai.ts`, `server/services/emailClarificationService.ts`, `server/services/debtorEmailService.ts`, `server/services/inboundReplyPipeline.ts`, `server/agents/prompts/collectionEmail.ts`, and 7 route files.

- 2026-04-07: Three related fixes from a persona-signature diagnostic where Mentzendorff/Cre8tive/RBC/FT Foods emails were rendering "Best regards, Datum Creative Media Limited, Datum Creative Media Limited" instead of "Sarah Mitchell / Credit Controller / Datum Creative Media Limited". Root cause was a chain of three bugs that only fired together. (1) `collectionsAgent.ts:116` `maxTokens` was 1024 — the new HTML-table emails (commit ab76fa17) are longer than the old plain-text emails and Claude was truncating mid JSON `body` string, causing `JSON.parse` to throw `Unterminated string in JSON at position …` for ~8% of generations. Raised to 2048, eliminated truncation in verification run. (2) When the LLM threw, `dailyPlanGenerator.ts:534` catch block kept the **template content** that had been written to `actions.content` before the LLM ran. The template comes from `charliePlaybook.ts EMAIL_TEMPLATES` whose sign-off was `{{senderName}}\n{{senderCompany}}` (two lines, no title). Both placeholders were bound from `tenantConfig` in `dailyPlanGenerator.ts:311-316` to `tenant.name` — the persona was never consulted by the fallback path. Fixed by loading the active persona via the same query as `collectionsAgent.resolvePersona()`, binding `senderName/senderTitle/senderCompany` from `persona.emailSignatureName/Title/Company`, adding `{{senderTitle}}` placeholder to all four EMAIL_TEMPLATES (FRIENDLY, FIRM, RECOVERY_FORMAL, INTEREST_INFO) between name and company, extending the `tenantConfig` parameter type on `prepareMessageFromDecision()`, and threading `senderTitle` into the `TemplateContext` interface and the context build. (3) The catch block previously left the broken-signature template body visible in the approval queue. Now it clears `content`, clears `subject`, sets `status = 'generation_failed'`, and stores the error in `metadata.generationError`. Approval queue routes filter on `pending_approval/pending` (`actionCentreRoutes.ts`) so `generation_failed` actions are correctly invisible. Verified end-to-end against the Datum production tenant: cleared queue, ran `generateDailyPlan` (47 contacts, 31 newly-created actions), all 31 went through the LLM path (`generatedBy: collections_agent_llm`), zero truncation failures, zero bad signatures, zero `generation_failed`, all 31 bodies contain "Sarah Mitchell" + "Credit Controller". Files: `server/agents/collectionsAgent.ts`, `server/services/dailyPlanGenerator.ts`, `server/services/charliePlaybook.ts`. Commit `4d72f8da`.
- 2026-04-07: Mandatory HTML invoice table + signature anti-substitution rule. The collection email LLM was rendering invoices inconsistently (sometimes table, sometimes bullet list, sometimes inline) and occasionally dropping the persona name+title from the sign-off. Tightened both prompt paths. `collectionEmail.ts buildSystemPrompt()` now contains an `INVOICE FORMATTING (MANDATORY)` block requiring an HTML table with exactly four columns (Invoice #, Amount, Due Date, Days Overdue) for any invoice reference, and instructing the LLM to emit the entire body as well-formed HTML (because `emailFormatter.ts` line 22 detects any block element and skips its plain-text-to-HTML conversion — so plain text mixed with a `<table>` would collapse on render). User prompt's per-invoice section now sends pipe-delimited rows the LLM converts directly into table rows. Signature block replaced with explicit `Line 1 — Name / Line 2 — Title / Line 3 — Company` format with anti-substitution wording ("never substituting or duplicating", "never replace the name line with the company name"). Defensive code-level dedupe skips the Company line if it equals the Name. `aiMessageGenerator.ts` (the fallback path) had a similar but conditional rule — made `invoiceTableInstruction` unconditional (was gated on `hasMultipleInvoices`), updated the example structure to show three labelled signature lines, changed `invoiceDetailsSection` gate from `> 1` to `>= 1`. Files: `server/agents/prompts/collectionEmail.ts`, `server/services/aiMessageGenerator.ts`. Commit `ab76fa17`.
- 2026-04-07: BUG FIX — Charlie bundling: `dailyPlanGenerator.ts` (the `/api/agent/run-now` path) was creating one bundled action per contact but never persisting the bundle to `actions.invoice_ids`. The full list lived in `metadata.allInvoiceIds` and the per-contact `action_summary` string ("Payment reminder - 4 outstanding invoices totalling £X"), but the actual array column stayed `NULL`. Every downstream consumer (`collectionsAgent.loadChaseInvoices`, `actionExecutor.buildMessageContext`, `actionExecutor.validateActionBeforeExecution`, `messagePreGenerator`) reads `action.invoiceIds?.length ? action.invoiceIds : [action.invoiceId]` — so with the array NULL they collapsed the bundle to the single `invoice_id` and the LLM generated an email demanding only that one invoice's balance. Symptom: Mentzendorff drawer showed "Chasing 1 invoice totalling £10,204" against the single £10,203.92 invoice, even though the action_summary said "4 outstanding invoices totalling £11,137.40". Fix: one-line addition at `dailyPlanGenerator.ts:469` — `invoiceIds: contactDecisions.map(d => d.invoiceId)` in the `proposeAction()` call, matching the pattern in `actionPlanner.ts:1289`. Verified: cleared queue, re-ran `generateDailyPlan` for Datum Creative Media tenant, Mentzendorff action now has all 4 invoice IDs in `invoice_ids` and the LLM-generated subject reads `Outstanding Invoices – £11,137.40`. The other planning path (`actionPlanner.ts`, used by adaptive scheduling) was already correct — only the daily-plan path was broken.
- 2026-04-07: PERF — Clear queue and Run agent now felt slow. Investigation showed `/api/approval-queue/clear` was already a single batch UPDATE (since commit 4b48fed7) — the perceived slowness was client-side refetch latency after `invalidateActionCentre()` triggered a full action-centre refetch. Fix: optimistic UI on both surfaces. `ApprovalsTab.tsx` clearQueueMutation: `onMutate` snapshots and zeroes the approvals list immediately, `onError` rolls back. `OverviewTab.tsx` clearQueueMutation: `onMutate` snapshots all `["/api/action-centre/summary"]` query data and zeroes the `queued` block (total/emails/sms/calls), `onError` restores. Run agent now is genuinely slow (LLM generation per debtor) — added start toast on `onMutate` ("Charlie is generating emails — may take a minute or two") and elapsed-time tracking in `onSuccess` so the completion toast reads "47 emails generated in 2m 14s". No server changes needed. Files: `client/src/components/action-centre/ApprovalsTab.tsx`, `client/src/components/action-centre/OverviewTab.tsx`.
- 2026-04-07: Brief credit-note language tightened. Removed `netOutstanding` field from `ConversationBriefData.creditBalance` and stripped all "net outstanding after credits" language from `conversationBriefService.ts`. Rationale: subtracting unallocated credits from the relationship total produces a misleading number — each invoice's amount already reflects credits allocated to it, and unallocated credits sit on the account until the debtor decides what to do with them. The brief now mentions credits as informational context only ("Unallocated credits of £X exist on this account. Do NOT subtract them from the chase amount or any invoice total"). Removed the `Reference net outstanding` constraint that fired on the no-chase-context path. Section heading renamed `CREDIT BALANCE` → `CREDIT NOTES`.
- 2026-04-07: BUG FIX — Collection email body still demanding payment of relationship-wide net (£30,325.63) instead of the bundled overdue amount (£10,203.92), even though `loadChaseInvoices` was loading the right bundle. Root cause was in `conversationBriefService.ts`: the brief output (a) `Total outstanding: £X` for the full account, and (b) hardcoded `You MUST reference the net amount (£X), NOT the gross invoice total` when the debtor had unapplied credits. Three reinforcing instructions all pointed at the relationship-wide net, drowning out the per-bundle `Total owed` in the user prompt. Fix: new optional `ChaseContext` parameter on `buildConversationBrief({ chaseAmount, chaseInvoiceCount, currency })`. When provided: (1) brief leads with a `CHASE TARGET FOR THIS EMAIL` section naming the bundle amount as the only figure to demand, (2) the relationship balance is renamed `ACCOUNT-WIDE CONTEXT (for awareness only — do not cite as the amount owed)`, (3) the credit-balance instruction inverts to "credits sit against the relationship, do NOT deduct from chase amount", (4) a `WHAT NOT TO DO` constraint reinforces "demand £X only, do NOT cite £Y". Cache key now includes chase amount so different bundles for the same contact don't collide. `collectionsAgent.ts` sequenced — chase invoices load first, then brief is built with computed chase context. `actionExecutor.ts` and `messagePreGenerator.ts` (the aiMessageGenerator fallback paths) also pass chase context derived from `action.invoiceIds`. The user prompt template `collectionEmail.ts` renamed `OUTSTANDING INVOICES` → `INVOICES TO CHASE IN THIS EMAIL`, leads with `AMOUNT TO DEMAND: £X (sum of the invoices below)`, lists each invoice with due date, and explicitly tells the LLM the subject line must reference the chase amount. Removed the per-prompt credit-balance netting block (was netting bundle total against full-account credits, mathematically incorrect). Files: `server/services/conversationBriefService.ts`, `server/agents/collectionsAgent.ts`, `server/agents/prompts/collectionEmail.ts`, `server/services/actionExecutor.ts`, `server/services/messagePreGenerator.ts`.
...
- **Xero token refresh race — FIXED (9 Apr)**: Two uncoordinated refresh mutex Maps (XeroAdapter.refreshLocks + xeroService.refreshInProgress) could both refresh the same rotating refresh token concurrently, causing `invalid_grant` and spurious "connection expired" states. Health check cron was also rotating tokens unconditionally every 20 minutes. Fixed via shared `withXeroRefreshLock(tenantId, fn)` in `server/services/xeroTokenLock.ts`, and the health check now verifies API access with the existing token first, only refreshing when actually expired.
...
- **Single entry point for debtor email generation**: `debtorEmailService.generateDebtorEmail()` is the ONE place where any email sent to a debtor gets persona-signed, chase-contextualised, and table-rendered. New outbound email paths must call it — do not resurrect static templates or reach for `aiMessageGenerator.generateEmail()` (dead). The `emailType` union (`'first_chase' | 'follow_up' | 'conversation' | 'clarification'`) dispatches to prompt addenda in `buildEmailTypeAddendum()`. Pass `invoiceIds: []` explicitly when the email should not render an invoice table.
- **`openai.ts` is legacy but not dead**: Five exports still serve live routes — `generateAiCfoResponse` (AI CFO chat via miscRoutes), `generateCollectionSuggestions` + `generateEmailDraft` (root routes.ts sidebar), `generateCollectionSms` (manual SMS draft via contactRoutes), and `generateCollectionEmail` (manual draft endpoint `/api/contacts/:id/generate-email` via dynamic import in contactRoutes). For any NEW debtor-facing automated email path, use `debtorEmailService.generateDebtorEmail()` — never re-add dead openai.ts functions (`analyzePaymentPatterns`, `detectSmsIntent` — both removed).
...
- **Two collection-email content paths share an `actions.content` row**: `dailyPlanGenerator.generateDailyPlanWithCharlie()` writes a Charlie-playbook **template body** into `actions.content` via `proposeAction()` BEFORE calling `processCollectionEmail()` (the LLM pipeline). The LLM is expected to overwrite `content` on success. If the LLM throws, the template body is what survives. Until the 7 April fix, the catch block left this template visible in the approval queue. The template uses `{{senderName}}/{{senderTitle}}/{{senderCompany}}` bound from `tenantConfig` in `dailyPlanGenerator.ts:311-326` — this config must load the active persona, not just `tenant.name`, or sign-offs will say the company name twice with no person attached.
- **`maxTokens` must be sized for the email format, not the persona prose**: When the prompt was changed to mandate an HTML invoice table with inline styles, the per-row HTML overhead (`<td style="border:1px solid #ddd;padding:8px;">`...) pushed average bodies past 1024 tokens. Claude truncated mid JSON `body` field, `JSON.parse` threw `Unterminated string in JSON at position …`, the LLM pipeline fell through, and the template body (with the wrong sign-off) was kept. Set in `collectionsAgent.ts:116`. Currently 2048 — revisit if the email format changes again or if multi-invoice bundles get larger.
- **`actions.status` is a free-form varchar, not an enum**: Schema comment at `schema.ts:861` lists allowed values but the column is `varchar`. New status values can be added without a migration, but consumers (approval queue routes, action centre, executor) must be checked. Approval queue routes filter on `inArray(actions.status, ["pending_approval", "pending"])` — anything else is invisible to the queue, which is why `generation_failed` is a safe value for hiding failed LLM generations.
